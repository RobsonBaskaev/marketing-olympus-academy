...
export default function SkillsDashboard() {
  ...
  useEffect(() => {
    ...
    setSkills([
      ...
      { name: "Стратегическая связность", score: strategyChoices === 4 && String(strategy.why || "").length >= 100 ? 2 : strategyChoices >= 2 ? 1 : 0, evidence: `${strategyChoices}/4 решений · обоснование ${Math.min(100, String(strategy.why || "").length)}/100`, next: "Свяжите сегмент, ценность, канал и бизнес-метрику.", href: "../strategy/" },
      { name: "Привлечение и бюджет", score: budget === 300000 && String(acquisition.reason || "").length >= 100 ? 2 : budget > 0 ? 1 : 0, evidence: `${budget.toLocaleString("ru-RU")} ₽ · обоснование ${Math.min(100, String(acquisition.reason || "").length)}/100`, next: "Соберите медиамикс под цель и объясните ограничения модели.", href: "../acquisition/" },
